...
This guide covers debugging the in-process agent execution system. Agents run directly via the Claude Agent SDK `query()` function within the BearClaw Node.js process — there are no containers, VMs, or Docker involved.
...
| **Conversations**     | `~/.bearclaw/var/agents/{folder}/conversations/*.md` | Daily archive written by the 1am rollover    |
