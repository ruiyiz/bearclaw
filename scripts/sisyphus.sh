#!/bin/bash
# Sisyphus — bearclaw health check monitor
# Runs independently via launchd, alerts via iMessage
# Named after the guy who never stops checking

set -euo pipefail

BEARCLAW_HOME="$HOME/.bearclaw"
DB_PATH="$BEARCLAW_HOME/store/messages.db"
STATE_FILE="$BEARCLAW_HOME/data/sisyphus_state.json"
IMSG="/opt/homebrew/bin/imsg"
SQLITE="/usr/bin/sqlite3"
ALERT_TO="+18046155370"
LOG_FILE="$HOME/Developer/Repos/oss/bearclaw/logs/sisyphus.log"
...
# --- Check 1: Is bearclaw process running? ---
check_process() {
	if ! pgrep -f "bearclaw/dist/index.js" >/dev/null 2>&1; then
		alerts+=("process_down|Bearclaw process is NOT running!")
		log "FAIL: bearclaw process not found"
	else
		log "OK: bearclaw process running"
	fi
}
