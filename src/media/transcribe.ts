...
const WHISPER_PATH =
  process.env.WHISPER_PATH || '/Users/ruiyiz/.local/bin/whisper';
const WHISPER_MODEL = process.env.WHISPER_MODEL || 'small';
const ELEVENLABS_API_KEY = process.env.ELEVENLABS_API_KEY;
const ELEVENLABS_STT_MODEL = process.env.ELEVENLABS_STT_MODEL || 'scribe_v1';
const TEMP_DIR = path.join(TMP_DIR, 'audio');
...
async function transcribeWithElevenLabs(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error(
        { status: response.status, errText, ms: Date.now() - t0 },
    ...
    if (transcription) {
      logger.info(
        { length: transcription.length, ms: Date.now() - t0 },
