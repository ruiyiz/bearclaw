...
Single Node.js process that connects to chat platforms (WhatsApp, Telegram, iMessage), the Gmail integration, and a local web UI, routing messages to the Codex Agent SDK running directly on the host. Each agent has its own working directory and memory.
